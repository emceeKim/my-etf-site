  ...
  const debates = [
    {
      ...
      content: `【거시분석가 AI】
PCE 인플레이션 2.7%로 목표치 2% 상회 중. 실업률 4.4%로 상승 추세. ISM 제조업 PMI 48~49 수축권 유지. 현재는 '경기 둔화 초입' 국면으로 판단. 방어 섹터 비중 확대가 필요한 시점.

【모멘텀 분석가 AI】
반박. PMI 수축이 맞지만 AI 반도체 수요는 PMI와 무관하게 독립적으로 폭증 중. NVDA·TSMC 실적은 경기 사이클을 이탈한 구조적 성장. 기술주 모멘텀 유지 주장.
...
세 모델 모두 부분적으로 맞음. 결론: 성장(QQQ) 50% + 방어(IEF) 30% + 현금 20% 혼합 포트폴리오가 현 국면에 최적.`
    },
    {
      ...
      content: `【거시분석가 AI】
경기 둔화 국면 섹터 로테이션 공식: 헬스케어(XLV) > 유틸리티(XLU) > 필수소비재(XLP) 순으로 방어 섹터 선호. 금리 인하 사이클 진입 시 채권(IEF·TLT)도 매력 상승.

【모멘텀 분석가 AI】
섹터 로테이션 공식은 AI 혁명 이전 시대 프레임. 2026년 현재 AI 인프라 투자는 경기 사이클과 무관. SOXX(반도체), QQQ(나스닥) 비중 확대가 정답. HBM4 양산 본격화로 SK하이닉스·마이크론 수혜 지속.

【리스크 관리자 AI】
모멘텀 편향 경고. AI가 좋다는 뉴스를 보고 AI ETF를 사는 것 자체가 이미 모멘텀 편향에 오염된 판단. 지표와 섹터가 같은 방향을 가리킬 때만 진입해야 함.
...
최종 조율: QQQ 40% + XLV 25% + IEF 25% + 현금 10%. 공격과 방어의 균형점.`
    },
    {
      ...
      content: `【거시분석가 AI】
지정학적 불확실성(트럼프 관세, 우크라이나, 대만) 고조 상황에서 방위산업 ETF(ITA·XAR) 주목. NATO 국방비 증가 트렌드 + 미국 국방예산 확대로 구조적 수혜.

【모멘텀 분석가 AI】
방위산업 동의. 단, 헬스케어(XLV)도 병행 필요. 고령화 + AI 신약 개발 가속으로 헬스케어는 경기 방어 + 성장 동시 포착 가능한 유일한 섹터.

【리스크 관리자 AI】
방위산업은 정책 리스크 큼. 트럼프 행정부의 예산 삭감 가능성 존재. 헬스케어가 더 안정적인 방어 수단. IEF(국채)가 진짜 헤지.

【검증자 AI】
결론: 방위산업 소량 편입은 OK, 하지만 핵심 헤지는 IEF. 헬스케어는 QQQ 보완재로 10~15% 편입 권고.`
    },
    {
      id: "r4",
      round: "Round 4",
      title: "최종 포트폴리오 합의",
      summary: "5라운드 토론 끝에 도출된 MIC AI 최종 결론",
...
▶ 최종 추천 포트폴리오
  1위 QQQ (나스닥 100) — 40%
     AI 구조적 성장, PMI와 무관한 독립 모멘텀
  2위 SPY (S&P 500) — 25%
     미국 경제 기초체력, 안정적 분산
  3위 IEF (미국 국채 7-10년) — 25%
     금리 인하 사이클 수혜, 진짜 헤지 수단
  + 현금 10% 유보 (변동성 대응)

▶ 핵심 원칙
  "지표와 섹터가 같은 방향을 가리킬 때만 진입"
  모멘텀 편향 경계 — 뉴스가 아닌 지표로 판단`
    }
  ];
  ...
  const indicators = [
    ...
    { name: "S&P500", value: "5,400", status: "보통", desc: "200일 이평선 위" },
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-56 bg-white border-r border-gray-100 flex flex-col">
        <div className="px-5 py-6 border-b border-gray-100">
          <div className="text-xs font-semibold text-blue-500 mb-1">MIC AI Labs</div>
          <div className="text-sm font-bold text-gray-900">ETF Insight</div>
        </div>
      ...
      <main className="flex-1 p-8 max-w-3xl">
        ...
        {activeMenu === "model" && (
          <div>
            <div className="mb-8">
              <span className="inline-block bg-blue-50 text-blue-500 text-xs font-semibold px-3 py-1 rounded-full mb-3">
                About MIC AI Labs
              </span>
              <h1 className="text-2xl font-bold text-gray-900 mb-2">MIC AI Model</h1>
              <p className="text-gray-400 text-sm">AI 멀티에이전트 토론으로 도출한 ETF 투자 판단 시스템</p>
            </div>

            <div className="mb-8">
              <h2 className="text-base font-bold text-gray-800 mb-4">현재 경제지표 현황</h2>
                ...
                {indicators.map((ind) => (
                  <div key={ind.name} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColor[ind.status]}`}>
                        {ind.status}
                      </span>
                    </div>
                    <div className="text-xl font-bold text-gray-900 mb-1">{ind.value}</div>
            ...
            <div className="mb-6">
              <h2 className="text-base font-bold text-gray-800 mb-2">ETF 판단 기준</h2>
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm mb-4">
                <p className="text-sm text-gray-600 leading-relaxed mb-3">
                  MIC AI Model은 4개의 AI 에이전트(거시분석가, 모멘텀 분석가, 리스크 관리자, 검증자)가 매월 토론을 통해 ETF를 선정합니다.
                </p>
            ...
            <div>
              <h2 className="text-base font-bold text-gray-800 mb-4">AI 멀티에이전트 토론 기록</h2>
                  ...
                  <div key={debate.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-sm font-bold text-gray-900">{debate.title}</span>
                        </div>
                    ...
                    {openDebate === debate.id && (
                      <div className="px-5 pb-5 border-t border-gray-50">
                        <pre className="text-xs text-gray-600 leading-relaxed whitespace-pre-wrap mt-4 font-sans">
                          {debate.content}
                        </pre>
                      </div>
                    )}
        ...
        {activeMenu === "etf" && (
          <div>
            <div className="mb-6">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-2">이달의 ETF</h1>
              <p className="text-gray-400 text-sm">AI 멀티에이전트 토론으로 선정한 2026년 4월 추천 ETF</p>
            </div>
            <div className="flex flex-col gap-3 max-w-xl">
              {[
                { rank: "1st", color: "bg-yellow-50 text-yellow-500", ticker: "QQQ", name: "Nasdaq 100 ETF", ret: "+18.4%", comment: "AI 구조적 성장, 반도체 섹터 주도 — 포트폴리오 40% 권고" },
                { rank: "2nd", color: "bg-blue-50 text-blue-400", ticker: "SPY", name: "S&P 500 ETF", ret: "+12.1%", comment: "미국 경제 연착륙 기대, 안정적 분산 — 포트폴리오 25% 권고" },
                { rank: "3rd", color: "bg-gray-100 text-gray-400", ticker: "IEF", name: "US Treasury 7-10yr ETF", ret: "+4.2%", comment: "금리 인하 사이클 수혜, 진짜 헤지 수단 — 포트폴리오 25% 권고" },
              ].map((etf) => (
                <div key={etf.ticker} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${etf.color.split(" ")[0]}`}>
                        <span className={`font-bold text-sm ${etf.color.split(" ")[1]}`}>{etf.rank}</span>
                      </div>
                      <div>
        ...
        {activeMenu === "returns" && (
          <div>
            <div className="mb-6">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-2">Returns Comparison</h1>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden max-w-xl">
              ...
              {[
                { ticker: "QQQ", name: "Nasdaq 100", m1: "+3.2%", m6: "+9.1%", y1: "+18.4%" },
                { ticker: "SPY", name: "S&P 500", m1: "+2.1%", m6: "+6.4%", y1: "+12.1%" },
                { ticker: "IEF", name: "US Treasury", m1: "+0.8%", m6: "+2.3%", y1: "+4.2%" },
              ].map((etf, i) => (
                <div key={etf.ticker} className={`grid grid-cols-4 px-5 py-4 items-center ${i < 2 ? "border-b border-gray-50" : ""}`}>
                  <div>
